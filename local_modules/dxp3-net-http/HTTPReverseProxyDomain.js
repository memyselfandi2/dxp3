...
class HTTPReverseProxyDomain extends EventEmitter {
	...
	addRule(_regexp, _group) {
		...
		if(_regexp instanceof RegExp) {
			_regexp.asString = _regexp.toString();
		...
		let regexpString = _regexp;
		_regexp = HTTPReverseProxyDomain.toRegExp(_regexp);
		_regexp.asString = regexpString;
		_regexp.group = _group;
		this.regexps.push(_regexp);
	}
	 ...
	 */
	addDestination(_group, _address, _port, _secure) {
		this.addHTTPServer(_group, _address, _port, _secure);
	}
	...
	addHTTPServer(group, address, port, secure) {
	 ...
	 * Alias for addHTTPServer(...)
	 */
	addServer(_group, _address, _port, _secure) {
		this.addHTTPServer(_group, _address, _port, _secure);
	}
	...
	_forwardRequest(request, response, pathname, serverDefinition) {
		...
		let creq = null;
		if(serverDefinition.secure) {
			// opts.agentOptions = {ca: fs.readFileSync(certificatesFolder + _domain + path.sep + 'fullchain.pem')};
			opts.rejectUnauthorized = false;
			creq = https.request(opts, (cres) => {
...
module.exports = HTTPReverseProxyDomain;