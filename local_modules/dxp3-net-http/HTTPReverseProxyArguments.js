 ...
 *
 * NAME
 * HTTPReverseProxyArguments
 */
const packageName = 'dxp3-net-http';
const moduleName = 'HTTPReverseProxyArguments';
...
/**
 * This class allows us to parse the different arguments an HTTPReverseProxy accepts.
 ...
 * </ul>
 *
 * @module dxp3-net-http/HTTPReverseProxyArguments
 */
 ...
 */
class HTTPReverseProxyArguments extends util.ConstructorArguments {
	/**
	 * Our constructor.
	 */
	constructor() {
	...
	static parseCommandLine() {
		let result = new HTTPReverseProxyArguments();
		...
		commandLineOption = commandLineOptions.createStringArray('destination',
																 'destinations,forwardto,forwardsto,server,servers,upstream',
																 'destination');
		commandLineOption.addHandler(function(result,index,propertyName) {
			index++;
			if(index < process.argv.length) {
...
module.exports = HTTPReverseProxyArguments;