...
const moduleName = 'SQLEqualCondition';
...
/**
 * @module dxp3-lang-sql/SQLEqualCondition
 */
...
class SQLEqualCondition extends SQLSingleCondition {
    /*********************************************
     * CONSTRUCTOR
     ********************************************/
    ...
    async evaluateWhere(_sqlTable) {
        ...
        logger.trace('evaluateWhere(...): Operandtype: ' + this.operandType);

        if(this.operandType === SQLSingleCondition.OPERANDTYPE.COLUMN_COLUMN) {
            result = await tableIndex.equalColumn(this._rightOperand.getName());            
        } else {
            result = await tableIndex.equal(this._rightOperand);
        }
...
module.exports = SQLEqualCondition;