...
const moduleName = 'SQLLessCondition';
...
/**
 * @module dxp3-lang-sql/SQLLessCondition
 */
...
class SQLLessCondition extends SQLSingleCondition {
    /*********************************************
     * CONSTRUCTOR
     ********************************************/
    ...
    async evaluateHaving(_rows) {
        ...
        if(this.operandType === SQLSingleCondition.OPERANDTYPE.COLUMN_COLUMN) {
            let _rightOperand = this._rightOperand.replaceAll(' ', '_');
    ...
    async evaluateWhere(_sqlTable) {
        ...
        if(this.operandType === SQLSingleCondition.OPERANDTYPE.COLUMN_COLUMN) {
            result = await tableIndex.lessColumn(this._rightOperand.getName());            
        } else {
            result = await tableIndex.less(this._rightOperand);
        }
        return result;
    }
...
module.exports = SQLLessCondition;