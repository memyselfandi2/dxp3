...
const moduleName = 'SQLGreaterCondition';
...
/**
 * @module dxp3-lang-sql/SQLGreaterCondition
 */
...
class SQLGreaterCondition extends SQLSingleCondition {
    /*********************************************
     * CONSTRUCTOR
     ********************************************/
    ...
    async evaluateWhere(_sqlTable) {
        ...
        if(this.operandType === SQLSingleCondition.OPERANDTYPE.COLUMN_COLUMN) {
            result = await tableIndex.greaterColumn(this._rightOperand.getName());            
        } else {
            result = await tableIndex.greater(this._rightOperand);
        }
        return result;
    }
...
module.exports = SQLGreaterCondition;