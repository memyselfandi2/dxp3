 ...
 *
 * NAME
 * SQLIncludesCondition
 */
const packageName = 'dxp3-lang-sql';
const moduleName = 'SQLIncludesCondition';
...
/**
 * @module dxp3-lang-sql/SQLIncludesCondition
 */
...
class SQLIncludesCondition extends SQLSingleCondition {
    /*********************************************
     * CONSTRUCTOR
     ********************************************/
    ...
    async evaluateHaving(_rows) {
        logger.trace('evaluateHaving(...): start.');
        ...
        if(this.operandType === SQLSingleCondition.OPERANDTYPE.COLUMN_COLUMN) {
            ...
            for(let i=0;i < _rows.length;i++) {
                let row = _rows[i];
                let leftValue = row[leftOperand];
                if(leftValue.includes(this._rightOperand)) {
                    result.push(row);
                }
            }
        }
        logger.trace('evaluateHaving(...): end.');
        return result;
    }
    ...
    async evaluateWhere(_sqlTable) {
        ...
        logger.trace('evaluateWhere(...): got table index: ' + tableIndex.getName());
        if(this.operandType === SQLSingleCondition.OPERANDTYPE.COLUMN_COLUMN) {
            result = await tableIndex.includesColumn(this._rightOperand.getName());            
        } else {
            result = await tableIndex.includes(this._rightOperand);
        }
    ...
    toString() {
        ...
        if(this._rightOperand === undefined) {
            result += 'undefined';
        } else if(this._rightOperand === null) {
            result += 'null';
        } else {
...
module.exports = SQLIncludesCondition;