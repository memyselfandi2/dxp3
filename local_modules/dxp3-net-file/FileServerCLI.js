...
const moduleName = 'FileServerCLI';
const path = require('path');
const canonicalName = packageName + path.sep + moduleName;
/**
 * @module dxp3-net-tcp/FileServerCLI
 */
const fs = require('fs');
...
/**
 * A FileServer command line interface program.
 */
class FileServerCLI {
    /*********************************************
     * CONSTRUCTOR
    ...
    /**
	 * @param {module:dxp3-net-tcp/FileServer} _fileServer
     */
	constructor(_fileServer) {
		this._fileServer = _fileServer;
	    this.originalPort = this._fileServer.getPort();
	    this.originalAddress = this._fileServer.getAddress();
	    this.originalTimeout = this._fileServer.getTimeout();
	...
	start() {
		this._rl = readline.createInterface({
			...
			prompt: 'DXP3-FILE-SERVER>'
		});
		this._rl.on('close', () => {
			this._parseExit();
		});
		this._fileServer.on(FileServerEvent.RUNNING, (_address, _port) => {
			console.log('FileServer running at ' + _address + ':' + _port);
			this._rl.prompt();
		});
		this._fileServer.on(FileServerEvent.STARTING, () => {
			console.log('FileServer starting...');
			this._rl.prompt();
		});
		this._fileServer.on(FileServerEvent.STOPPED, () => {
			console.log('FileServer stopped.');
			this._rl.prompt();
		});
		this._fileServer.on(FileServerEvent.STOPPING, () => {
			console.log('FileServer stopping...');
			this._rl.prompt();
		});
		this._fileServer.on(FileServerEvent.ERROR, (_error) => {
			console.log('FileServer error: ' + _error.toString());
			this._rl.prompt();
		});
		console.clear();
		console.log('');
		console.log('Use this interface to control a FileServer.');
		console.log('The FileServer has not yet been started, but it has been');
				...
				switch(command) {
					...
					case 'tcpaddress':
						if(commandArguments.length > 0) {
							...
							console.log('What address should the FileServer bind to?');
	...
	_parseHelp() {
		...
		// Clear the window.
		console.clear();
		console.log('CONFIGURATION / PREFERENCES: Commands to control the FileServer.');
		console.log('');
		...
		console.log('                                     | tcpaddress           | the FileServer to all ip addresses of the host. |');
		console.log('set port            <number>         + port,                + Set the TCP port to bind to.                   |');
		console.log('                                     | tcpport              | The FileServer must be stopped first before     |');
		console.log('                                     |                      | the port can be updated.                       |');
		console.log('set timeout         <number>         + timeout              + Set the connection timeout.                    |');
		console.log('status                               + config,              + Get the current state of the FileServer.        |');
		...
		console.log('-------------------------------------+----------------------+------------------------------------------------+');
		console.log('');
		console.log('PROCESS FLOW: Commands to start and stop the FileServer.');
		console.log('');
		console.log('COMMAND -----------------------------+ ALIASES -------------+ DESCRIPTION -----------------------------------+');
		console.log('start                                +                      + Start the FileServer.                           |');
		console.log('stop                                 +                      + Stop the FileServer.                            |');
		console.log('-------------------------------------+----------------------+------------------------------------------------+');
		console.log('');
	...
	async _parseLoad(line) {
		...
		if(!this._fileServer.isInitialized() &&
		   !this._fileServer.isStopped()) {
			console.log('Make sure the FileServer is not running. Cancelled.');
			this._rl.prompt();
			return;
		}
		if(this._hasUnsavedChanges) {
			this._rl.question('DXP3-FILE-SERVER> There are unsaved changes. Are you sure you want to continue loading?', (answer) => {
	...
	async _load(line) {
		try {
			...
			this._fileServer.setAddress(persist.address);
			this._fileServer.setPort(persist.port);
			this._fileServer.setTimeout(persist.timeout);
	...
	_parseReset(line) {
	    ...
	    if(line === 'yes' || line === 'y' || line === 'true') {
	    	try {
		    	this._fileServer.setPort(this.originalPort);
		    	this._fileServer.setAddress(this.originalAddress);
		    	this._fileServer.setTimeout(this.originalTimeout);
				this._hasUnsavedChanges = false;
		    	console.log('Reset.');
		    } catch(_exception) {
				if(_exception.code === FileError.ILLEGAL_STATE.code) {
					console.log('Unable to completely reset, because the FileServer is running.')
					console.log('Make sure to first stop the FileServer before attempting another reset.');
				} else {
	...
	async _parseSave(line) {
		...
		let persist = {};
		persist.address = this._fileServer.getAddress();
		persist.port = this._fileServer.getPort();
		persist.timeout = this._fileServer.getTimeout();
		try {
	...
	_parseSetAddress(line) {
		...
		try {
			this._fileServer.setAddress(line);
			console.log('Address set to ' + this._fileServer.getAddress());
			this._hasUnsavedChanges = true;
		} catch(_exception) {
				if(_exception.code === FileError.ILLEGAL_STATE.code) {
					console.log('Unable to update the address, because the FileServer is running.')
					console.log('Make sure to first stop the FileServer before updating the address.');
				} else {
					console.log('Error: ' + _exception.toString());
				}
		}
	...
	_parseSetPort(line) {
		...
		try {
			this._fileServer.setPort(line);
			console.log('Port set to ' + this._fileServer.getPort());
			this._hasUnsavedChanges = true;
		} catch(_exception) {
			if(_exception.code === FileError.ILLEGAL_ARGUMENT.code) {
				console.log('The supplied port is not a number. Unable to update the TCP port.')
			} else if(_exception.code === FileError.ILLEGAL_STATE.code) {
				console.log('Unable to update the port, because the FileServer is running.')
				console.log('Make sure to first stop the FileServer before updating the port.');
			} else {
				console.log('Error: ' + _exception.toString());
			}
		}
	...
	_parseSetTimeout(line) {
		...
		try {
			this._fileServer.setTimeout(line);
			console.log('Timeout set to ' + this._fileServer.getTimeout());
			this._hasUnsavedChanges = true;
		} catch(_exception) {
			if(_exception.code === FileError.ILLEGAL_ARGUMENT.code) {
	...
	_parseStart() {
		if(this._fileServer.isRunning()) {
			console.log('The FileServer is already running.');
			this._rl.prompt();
			return;
		}
		if(this._fileServer.isStarting()) {
			console.log('The FileServer is already starting.');
			this._rl.prompt();
			return;
		}
		try {
	...
	_parseStatus() {
		console.log('Address: ' + this._fileServer.getAddress());
		console.log('Port   : ' + this._fileServer.getPort());
		console.log('Timeout: ' + this._fileServer.getTimeout());
		console.log('State  : ' + this._fileServer.getState());
		console.log('Logging: ' + logging.Level.toString(logging.getLevel()));
		this._rl.prompt();
	}

	_parseStop() {
		if(this._fileServer.isStopped()) {
			console.log('The FileServer is already stopped.');
			this._rl.prompt();
			return;
		}
		if(this._fileServer.isStopping()) {
			console.log('The FileServer is already stopping.');
			this._rl.prompt();
			return;
		}
		try {
	...
	static main() {
		try {
			...
			let fileServerCLI = new FileServerCLI(fileServer);
			fileServerCLI.start();
		} catch(_exception) {
